...
def check_image(image):
    assertion = tf.assert_equal(tf.shape(image)[-1],
                                3,
...
def lab_to_rgb(lab):
    with tf.name_scope("lab_to_rgb"):
        ...
        with tf.name_scope("xyz_to_srgb"):
            ...
            exponential_mask = tf.cast(rgb_pixels > 0.0031308,
                                       dtype=tf.float32)
...
def create_model(inputs, targets):
    ...
    with tf.name_scope("generator_loss"):
        ...
        gen_loss_L1 = tf.reduce_mean(
            tf.abs(targets - outputs))  # tf.nn.l2_loss(targets - outputs)
    ...
    if a.mode == "export":
        ...
        input_image = tf.cond(
            tf.equal(tf.shape(input_image)[2],
                     4), lambda: input_image[:, :, :3], lambda: input_image)
        # convert grayscale to RGB
        input_image = tf.cond(tf.equal(tf.shape(input_image)[2],
                                       1), lambda: tf.image.grayscale_to_rgb(
                                           input_image), lambda: input_image)
